...
The first step to migrate hsiams v2.4.x to v3.0 is upgrade the Bootstrap 3 base code to Bootstrap 4, the full instruction [here](https://getbootstrap.com/docs/4.4/migration/) after you upgraded the base code you need to update the markups.

#### CSS / JS Files

Since hsiams v3.0 Bootstrap 4 is complete included in hsiams's CSS file. You will not need to load the Bootstrap CSS file `bootstrap(.min).css`, but you will need the Bootstrap JS file `bootstrap(.min).js`.
...
2. Logo / Brand Link
  - `<a href="index3.html" class="brand-link"><img src="dist/img/adminlteLogo.png" alt="hsiams Logo" class="brand-image img-circle elevation-3" style="opacity: .8"><span class="brand-text font-weight-light">hsiams 3</span></a>` replaces the old logo
3. Sidebar
...
The biggest change in content header is hsiams use here now `.container-fluid`, `.row` & `.col-*` and the breadcrumb markup changed, here are all changes:
